...
mercury = Body(
    pos=[5.79e10, 0.0, 0.0], vel=[0.0, 47400.0, 0.0], mass=3.301e23, radius=2.44e6
)
venus = Body(
    pos=[1.082e11, 0.0, 0.0], vel=[0.0, 35000.0, 0.0], mass=4.867e24, radius=6.052e6
)
earth = Body(
    pos=[1.496e11, 0.0, 0.0], vel=[0.0, 29780.0, 0.0], mass=5.972e24, radius=6.371e6
)
mars = Body(
    pos=[2.279e11, 0.0, 0.0], vel=[0.0, 24100.0, 0.0], mass=6.417e23, radius=3.389e6
)
...
with HDF5Writer(
    ...
    gravity = GravityField(
        theta=0.5
    )  # Create a gravity field with a softening parameter (theta) to prevent singularities
    sim = Simulation(
        bodies=bodies,
        field=gravity,
        integrator=rk4,
